...
typedef ct::rbd::FloatingBaseFDSystem<ct::rbd::Walker::tpl::Dynamics<SCALAR>, false> WalkerSystemAD;

// extract dimensions
const size_t state_dim = WalkerSystemAD::STATE_DIM;
const size_t control_dim = WalkerSystemAD::CONTROL_DIM;


void generateInverseDynamics()
{
    typedef ct::core::DerivativesCppadCG<state_dim + 18, control_dim + 6> JacCG;
    typename JacCG::FUN_TYPE_CG f = ct::models::Walker::walkerInverseDynamics<SCALAR>;
    JacCG jacCG(f);

    try
    {
        std::cout << "Generating Jacobian of Inverse Dynamics wrt state using forward mode... " << std::endl;
        jacCG.generateJacobianSource("WalkerInverseDynJacForward", ct::models::WALKER_CODEGEN_OUTPUT_DIR,
            ct::core::CODEGEN_TEMPLATE_DIR, "models", "Walker", JacCG::Sparsity::Ones(), false);

        std::cout << "Generating Jacobian of Inverse Dynamics wrt state using reverse mode... " << std::endl;
        jacCG.generateJacobianSource("WalkerInverseDynJacReverse", ct::models::WALKER_CODEGEN_OUTPUT_DIR,
            ct::core::CODEGEN_TEMPLATE_DIR, "models", "Walker", JacCG::Sparsity::Ones(), true);
...
void generateForwardKinematics()
{
    ...
    typename JacCG::FUN_TYPE_CG f = ct::models::Walker::walkerForwardKinematics<SCALAR>;
    JacCG jacCG(f);

    try
    {
        std::cout << "Generating Jacobian of Forward Kinematics wrt state using forward mode... " << std::endl;
        jacCG.generateJacobianSource("WalkerForwardKinJacForward", ct::models::WALKER_CODEGEN_OUTPUT_DIR,
            ct::core::CODEGEN_TEMPLATE_DIR, "models", "Walker", JacCG::Sparsity::Ones(), false);

        std::cout << "Generating Jacobian of Forward Kinematics wrt state using reverse mode... " << std::endl;
        jacCG.generateJacobianSource("WalkerForwardKinJacReverse", ct::models::WALKER_CODEGEN_OUTPUT_DIR,
            ct::core::CODEGEN_TEMPLATE_DIR, "models", "Walker", JacCG::Sparsity::Ones(), true);
    } catch (const std::runtime_error& e)
    {
...
void generateForwardZeroForwardDynamics()
{
    ...
    // auto asd = ct::models::Walker::walkerContactModelForwardDynamics<SCALAR>(testInput);
    typename JacCG::FUN_TYPE_CG f = ct::models::Walker::walkerContactModelForwardDynamics<SCALAR>;
    JacCG jacCG(f);

    try
    {
        std::cout << "Generating Forward Zero Code... " << std::endl;
        jacCG.generateForwardZeroSource("WalkerForwardZero", ct::models::WALKER_CODEGEN_OUTPUT_DIR,
            ct::core::CODEGEN_TEMPLATE_DIR, "models", "Walker", false);
    } catch (const std::runtime_error& e)
    {
...
void generateFDLinearization(int argc, char* argv[])
{
    ...
    typedef ct::rbd::EEContactModel<typename WalkerSystemAD::Kinematics> ContactModel;

    std::shared_ptr<WalkerSystemAD> adSystem = std::shared_ptr<WalkerSystemAD>(new WalkerSystemAD);
    ...
    std::string baseName;
    if (useContactModel)
        baseName = "WalkerWithContactModelLinearized";
    else
        baseName = "WalkerBareModelLinearized";
    ...
    try
    {
        ...
        if (argc > 1 && std::string(argv[1]).compare("reverse") == 0)
        {
            ...
            adLinearizer.generateCode(baseName + "Reverse", ct::models::WALKER_CODEGEN_OUTPUT_DIR,
                ct::core::CODEGEN_TEMPLATE_DIR, "models", "Walker", true);
        }
        else
        {
            std::cout << "generating using forward mode" << std::endl;

            adLinearizer.generateCode(baseName + "Forward", ct::models::WALKER_CODEGEN_OUTPUT_DIR,
                ct::core::CODEGEN_TEMPLATE_DIR, "models", "Walker", false);
        }
