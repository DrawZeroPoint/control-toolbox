...
#define IKFAST_NAMESPACE franka_ik
#include <ikfast.h>
#include <ct/core/core.h>
#include <ct/rbd/state/JointState.h>
#include <ct/models/Franka/FrankaJointLimits.h>
...
class FrankaInverseKinematics : public InverseKinematicsBase<7, SCALAR>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW

    using BASE = InverseKinematicsBase<7, SCALAR>;
    ...
    FrankaInverseKinematics() = default;
        ...
        const std::vector<size_t>& freeJoints = std::vector<size_t>()) override
    {
        ...
        if (size_t(franka_ik::GetNumFreeParameters()) != freeJoints.size())
            throw std::runtime_error("Error specifying free joints in FrankaInverseKinematics");
        ...
        franka_ik::ComputeIk(eeBasePose.position().toImplementation().data(), eeBaseRotationRowMajor.data(),
        ...
        for (size_t i = 0; i < num_solutions; i++)
        {
            ...
            sol.toUniquePosition(ct::models::Franka::jointLowerLimit());
            if (sol.checkPositionLimits(ct::models::Franka::jointLowerLimit(), ct::models::Franka::jointUpperLimit()))
